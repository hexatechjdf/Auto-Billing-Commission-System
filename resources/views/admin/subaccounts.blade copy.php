            ...
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    <div class="d-flex align-items-center">
                        <div class="me-3">
                            <input type="text" id="subaccounts-search" class="form-control"
                                placeholder="Search subaccounts...">
                        </div>
                        {{--
                        <div>
                            <select id="subaccounts-length" class="form-select">
                ...
                <div class="modal-body">
                    <form id="edit-subaccount-form">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" id="stripe-payment-method-id" class="form-control">
                        </div>

                        <div class="mb-3">
                            <label class="form-label">Stripe Customer Id</label>
                            <input type="text" id="stripe-customer-id" class="form-control">
                        </div>
...
@push('scripts')
    ...
    <script>
        ...
        function populateSubaccountsTable(subaccounts) {
            ...
            subaccounts.forEach(subaccount => {
                const row = `
                    <tr>
                        ...
                        <td>${subaccount.stripe_payment_method_id || 'N/A'}</td>
                        <td>${subaccount.stripe_customer_id || 'N/A'}</td>
                        ...
                        {{-- <td>${subaccount.pause_at || 'N/A'}</td>
                        <td>${subaccount.price_id || 'N/A'}</td>
                        <td>${subaccount.last_checked_at || 'N/A'}</td>
                        <td>${subaccount.created_at || 'N/A'}</td>
                        --}}
                        <td>
            ...
            dataTable = $('#subaccounts-table').DataTable({
                responsive: true,
                pageLength: 20,
                order: [
                    [0, 'asc']
                ], // Sort by Location ID by default
                searching: true, // Enable searching for custom input
                dom: 'rt<"d-flex justify-content-between"i p>', // Custom layout for info and pagination
                columnDefs: [{
                        orderable: false,
                        targets: [2, 3, 6]
                    } // Disable sorting on Chargeable, Allow Uninstall, Actions
                ]
            });
            ...
            // Bind custom page length selection
            $('#subaccounts-length').on('change', function() {
        ...
        function toggleSubaccount(id, field, value) {
            ...
            $.ajax({
                ...
                data: {
                    id: id,
                    field: field,
                    value: value ? 1 : 0 // Send 0 or 1 to avoid serialization issues
                },
                success: function(response) {
                    if (response.success) {
        ...
        function editSubaccount(id) {
            ...
            $('#stripe-payment-method-id').val(subaccount.stripe_payment_method_id);
            $('#stripe-customer-id').val(subaccount.stripe_customer_id);
        ...
        function saveSubaccount() {
            ...
            const stripePaymentMothedId = $('#stripe-payment-method-id').val();
            const stripeCustomerId = $('#stripe-customer-id').val();
