...
use App\Jobs\ProcessStripeIntentWebhook;
use App\Jobs\ProcessStripePaymentMethodWebhook;
...
class HandlerService
{
    ...
    public function handleGhlAppUninsalllWebhook(array $payload, array $log)
    {
    ...
    public function handleGhlLocationConnectWebhook(array $payload, array $log)
    {
        $locationId  = $payload['locationId'] ?? null;
        ...
        ProcessGhlLocationWebhook::dispatch($payload)->delay(now()->addMinutes(3)); // ->delay(now()->addMinutes(3)); //TODO: dispatch
        return response()->json(['success' => true, 'message' => 'Webhook queued for processing']);
    }

    public function handleGhlLocationWebhook(string $webhookType, array $payload, array $log)
    {
    ...
    public function handleGhlOrderStatusUpdate(string $webhookType, array $payload, array $log)
    { // TODO: optimize
        ...
        if ($locationId == $primarySubaccount) {
            ...
            ProcessPrimaryOrderJob::dispatch($payload)->delay(now()->addMinute(2)); //->delay(now()->addMinute(2)); // // TODO: dispatch
        } else {
            ProcessGhlOrderStatusUpdate::dispatch($payload); // TODO: dispatch
        }

        return response()->json(['success' => true, 'message' => 'Webhook queued for processing']);
    }

    public function handleGhlInvoicePaid(string $webhookType, array $payload, array $log)
    {
    ...
    public function handleStripePaymentMethodWebhook(string $webhookType, array $payload, array $log)
    {
        ProcessStripePaymentMethodWebhook::dispatch($payload); //TODO: dispatch
        return response()->json(['success' => true, 'message' => 'Webhook queued for processing']);
    }

    public function handleStripePaymentIntentWebhook(string $webhookType, array $payload, array $log)
    {
        ...
        ProcessStripeIntentWebhook::dispatch($payload); //TODO: dispatch
        return response()->json(['success' => true, 'message' => 'Webhook queued for processing']);
    }
